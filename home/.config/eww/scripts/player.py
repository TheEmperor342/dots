...
if sys.argv[1] == '--title':
  ...
  link = subprocess.getoutput("playerctl metadata mpris:artUrl --player=spotify")

  if link == "No players found" or not useAlbumArt: print("images/spotify.svg")
  ...
  isNotPlaying = subprocess.getoutput("playerctl status --player=spotify") == "No players found"
  directory = os.listdir("albumart")
  if isNotPlaying or not useAccentColor: 
    print("Nothing playing")
  else:
    colorthief = __import__("colorthief").ColorThief(f"albumart/{directory[0]}")
